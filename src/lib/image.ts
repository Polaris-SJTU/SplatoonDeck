...
export function generateMacro(pixels: Uint8Array, width: number, height: number, options: MacroOptions): MacroResult {
  ...
  const pressMs = Math.max(80, Math.min(200, Math.round(options.pressDurationMs)));
  ...
  // Always walk the cursor back to the top-left corner (0, 0).
  // L3 clear does NOT reset cursor position, and on resume the cursor
  // could be anywhere on the canvas. Walking to the wall guarantees a
  // known starting point for all subsequent positioning.
  loopTap('DPAD_LEFT', width - 1);
  loopTap('DPAD_UP', height - 1);
  if (scanDirection === 'row') {
  ...
  if (scanDirection === 'row') {
    ...
    for (let y = startBand; y <= endBand; y++) {
      ...
      const targetX = direction > 0 ? firstBlack[y] : lastBlack[y];
      currentX = moveX(currentX, targetX);
      const scanEnd = direction > 0 ? lastBlack[y] : firstBlack[y];
      const scanLen = Math.abs(scanEnd - currentX) + 1;
      for (let step = 0; step < scanLen; step++) {
       ...
       if (step < scanLen - 1) {
         moveTap(direction > 0 ? 'DPAD_RIGHT' : 'DPAD_LEFT');
          currentX += direction;
        }
      }
      if (y < endBand) moveTap('DPAD_DOWN');
    ...
    for (let x = startBand; x <= endBand; x++) {
      ...
      const targetY = direction > 0 ? firstBlack[x] : lastBlack[x];
      currentY = moveY(currentY, targetY);
      const scanEnd = direction > 0 ? lastBlack[x] : firstBlack[x];
      const scanLen = Math.abs(scanEnd - currentY) + 1;
      for (let step = 0; step < scanLen; step++) {
        const index = currentY * width + x;
        if (pixels[index]) { tap('A'); plannedPixels[index] = 1; pixelTimestamps.push(totalMs - preparationDurationMs); }
        if (step < scanLen - 1) {
          moveTap(direction > 0 ? 'DPAD_DOWN' : 'DPAD_UP');
          currentY += direction;
        }
      }
      if (x < endBand) moveTap('DPAD_RIGHT');
