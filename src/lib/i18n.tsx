...
const en: Record<string, string> = {
  ...
  '需要重启 Windows': 'Windows restart required', '系统组件已启用。重启后再次点击“继续安装”即可，不会重复下载。': 'System components are enabled. Restart Windows, then select Continue Installation; files will not be downloaded again.', '已恢复上次会话': 'Previous session restored', '检测到蓝牙仍由 WSL 接管，可点击“归还蓝牙”安全恢复 Windows 蓝牙。': 'Bluetooth is still attached to WSL. Return it safely to restore Windows Bluetooth.',
  '运行环境': 'Runtime environment', '检查 / 修复依赖': 'Check / Repair Dependencies', '重启后继续安装': 'Continue After Restart', '一键安装依赖': 'Install Dependencies', '安装需要管理员确认和网络连接；首次启用 WSL 可能要求重启。': 'Setup requires administrator confirmation and internet access. Enabling WSL for the first time may require a restart.',
  ...
  '不留下一滴墨水': 'Leave no ink behind', '清理只移除本应用创建的 Linux 环境、USB 共享记录和由它安装的 usbipd。发现其他软件在使用 WSL 时，共享系统功能会保留。': 'Cleanup removes only the Linux environment, USB sharing records, and usbipd installed by this app. Shared system features remain when another app uses WSL.', '正在清理…': 'Cleaning…', '卸载应用依赖': 'Uninstall App Dependencies',
...
const ja: Record<string, string> = {
  ...
  '需要重启 Windows': 'Windows の再起動が必要です', '系统组件已启用。重启后再次点击“继续安装”即可，不会重复下载。': 'システム機能を有効にしました。再起動後に「インストールを続行」を押してください。再ダウンロードは不要です。', '已恢复上次会话': '前回のセッションを復元しました', '检测到蓝牙仍由 WSL 接管，可点击“归还蓝牙”安全恢复 Windows 蓝牙。': 'Bluetooth が WSL に接続されたままです。「Windows に戻す」で安全に復元できます。',
  '运行环境': '実行環境', '检查 / 修复依赖': '依存関係を確認 / 修復', '重启后继续安装': '再起動後に続行', '一键安装依赖': '依存関係をインストール', '安装需要管理员确认和网络连接；首次启用 WSL 可能要求重启。': 'セットアップには管理者確認とネット接続が必要です。WSL の初回有効化では再起動が必要な場合があります。',
  ...
  '不留下一滴墨水': '跡を残さずクリーンアップ', '清理只移除本应用创建的 Linux 环境、USB 共享记录和由它安装的 usbipd。发现其他软件在使用 WSL 时，共享系统功能会保留。': 'このアプリが作成した Linux 環境、USB 共有記録、usbipd だけを削除します。他のアプリが WSL を使用している場合、共有システム機能は残します。', '正在清理…': '削除中…', '卸载应用依赖': 'アプリ依存関係を削除',
