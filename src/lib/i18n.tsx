...
const ja: Record<string, string> = {
  ...
  '绘制已中止': '描画を中断しました', '根据停止时进度，建议从第 {{position}} {{unit}}附近续画；请对照游戏画面修正。': '停止位置から、{{unit}} {{position}} 付近での再開を推奨します。ゲーム画面と比較して調整してください。', '使用第 {{position}} {{unit}}': '{{unit}} {{position}} を使用', '预计耗时': '予想時間', '绘制范围': '描画範囲', '启动准备': '開始準備', '全自动': '全自動', '跳过空白': '空白をスキップ', '内容范围': '内容範囲', '扫描方向': 'スキャン方向', '行': '行', '列': '列', '正在准备画布…': 'キャンバスを準備中…', '正在喷墨…': '描画中…', '剩余时间': '残り時間', '停止绘制': '描画を停止', '正在发送绘制指令…': '描画コマンドを送信中…', '开始自动绘制': '自動描画を開始', '连接手柄后开始': 'コントローラー接続後に開始', '每次方向移动都包含独立按下与松开帧；逐行固定从左边界归位，逐列固定从顶部边界归位。预览和绘制共用同一份 320 × 120 二值矩阵。': '各方向移動には独立した押下と解放があります。行描画は左端、列描画は上端から毎回原点復帰します。プレビューと描画は同じ 320 × 120 の二値行列を使用します。',
