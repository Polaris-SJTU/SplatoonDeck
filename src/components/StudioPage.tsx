...
import { CANVAS_HEIGHT, CANVAS_WIDTH, createCalibrationPixels, DitherMode, FitMode, formatDuration, generateMacro, ImageSettings, processImage } from '../lib/image';
import { drawingScanPercent, estimateResumeRow } from '../lib/drawing-state';
...
function RowRange({ label, value, min, max, disabled, onChange }: { label: string; value: number; min: number; max: number; disabled: boolean; onChange(value: number): void }) {
  ...
  return <div className="row-range-control">
    <div><span>{label}</span><label><input aria-label={label} disabled={disabled} type="number" min={min} max={max} value={value} onChange={(event) => update(Number(event.target.value))} /><em>/ {CANVAS_HEIGHT}</em></label></div>
    <input disabled={disabled} type="range" min={min} max={max} value={value} onChange={(event) => update(Number(event.target.value))} />
  </div>;
}

function PixelPreview({ pixels }: { pixels: Uint8Array }) {
...
export default function StudioPage({ connection, progress, onNeedController, notify }: Props) {
  const [source, setSource] = useState<HTMLImageElement | null>(null);
  const [fileName, setFileName] = useState('');
  const [settings, setSettings] = useState(defaultSettings);
  const [pixels, setPixels] = useState<Uint8Array | null>(null);
  const [speed, setSpeed] = useState(60);
  const [cautious, setCautious] = useState(true);
  ...
  const macro = useMemo(() => pixels ? generateMacro(pixels, CANVAS_WIDTH, CANVAS_HEIGHT, {
    pressDurationMs: speed, cautious, autoSave: false, startRow: startRow - 1, endRow: endRow - 1
  }) : null, [pixels, speed, cautious, startRow, endRow]);
  ...
  const elapsedMs = macro ? (progress ?? 0) * macro.durationMs : 0;
  const drawingDurationMs = macro ? Math.max(1, macro.durationMs - macro.preparationDurationMs) : 1;
  const drawingProgress = macro ? Math.max(0, Math.min(1, (elapsedMs - macro.preparationDurationMs) / drawingDurationMs)) : 0;
  const preparing = running && !!macro && elapsedMs < macro.preparationDurationMs;
  const remainingMs = macro ? Math.max(0, macro.durationMs - elapsedMs) : 0;
  const scanTop = drawingScanPercent(startRow, endRow, drawingProgress, CANVAS_HEIGHT);
  const stop = async () => {
      ...
      <div className="studio-layout">
        ...
        <article className="card preview-panel">
          ...
          <div className={`pixel-canvas ${pixels ? '' : 'empty'}`}>
            ...
            {pixels ? <PixelPreview pixels={pixels} /> : <div><span>⌁</span><strong>等待你的作品</strong><small>左侧导入图片后会在这里实时预览</small></div>}
            {running && !preparing && <div className="drawing-scan" style={{ top: `${scanTop}%` }} />}
          </div>
          ...
          <div className={`draw-config ${parametersLocked ? 'controls-locked' : ''}`}>
            ...
            <Range disabled={parametersLocked} label="按键间隔 (ms)" value={speed} min={40} max={120} onChange={setSpeed} />
            <RowRange label="续画起始行" value={startRow} min={1} max={CANVAS_HEIGHT} disabled={parametersLocked} onChange={changeStartRow} />
            <RowRange label="本批结束行" value={endRow} min={1} max={CANVAS_HEIGHT} disabled={parametersLocked} onChange={changeEndRow} />
            <label className="toggle-row"><span><strong>逐行边界复位</strong><small>每行扫满 320 个坐标，并在换行前再次压紧边界</small></span><input disabled={parametersLocked} type="checkbox" checked={cautious} onChange={(event) => setCautious(event.target.checked)} /><i /></label>
            ...
            <div className="draw-summary"><span>预计耗时<strong>{macro ? formatDuration(macro.durationMs) : '—'}</strong></span><span>绘制范围<strong>{startRow}–{endRow} 行</strong></span><span>启动准备<strong>全自动</strong></span></div>
            ...
            <p className="fine-print center">预览与绘制共用同一份 320 × 120 二值矩阵；路径逐行访问每个像素坐标并在发送前完成一致性校验。不会自动发布作品。</p>
          </div>
        </article>
      </div>
    </section>
