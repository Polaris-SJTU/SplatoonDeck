...
`v0.3.0` では、コントローラー操作マクロの録画と、回数指定または連続ループ再生を追加し、環境管理を再開可能な段階式フローへ刷新しました。セットアップには手順・進捗・コマンド出力が表示され、ウィンドウを閉じると子プロセスも停止します。専用 Linux 環境の作成と初回起動は現在の WSL に対応し、Ubuntu 環境のダウンロードは再開、自動再試行、SHA-256 検証に対応します。削除時は SplatoonDeck が追加したコンポーネントだけを取り除き、既存または共有中の WSL 環境を保持します。詳しくは [v0.3.0 リリースノート](https://github.com/Polaris-SJTU/SplatoonDeck/releases/tag/v0.3.0)をご覧ください。

### 初めて Switch 2 に接続する

1. SplatoonDeck を起動し、**セットアップ**を開きます。左下の言語セレクターで簡体字中国語、English、日本語をいつでも切り替えられ、選択内容は自動で保存されます。
2. **依存関係を確認 / 修復**を選びます。最初に WSL、Windows 機能、Bluetooth コンポーネントを準備します。再起動を求められた場合は Windows を再起動し、SplatoonDeck を開いて **インストールを続行**を選ぶと次の段階から再開します。
...
1. Switch 2 に接続し、仮想コントローラー画面の下部にある **マクロ録画・再生**を開きます。
2. **録画開始**を押し、画面上のコントローラー、キーボード、またはマウスで一度操作します。ボタン状態、スティック位置、操作間隔が記録されます。
3. **録画停止**を押して端末内に保存します。アプリを再起動しても録画は利用できます。
4. 再生回数または **停止まで繰り返す**を選び、**再生**を押します。録画時と同じテンポで操作が繰り返されます。
5. 長時間再生や無限ループは **再生停止**でいつでも終了できます。再生中は他のコントローラー入力がロックされます。
...
- ボタン、左右スティック、操作間隔を録画し、回数指定または手動停止まで繰り返し再生可能。
...
- セットアップと削除は同時に一つだけ実行され、進捗ウィンドウを閉じると、起動した DISM、winget などの子プロセスも停止します。
- Ubuntu 環境のダウンロードは再開、4 回の自動再試行、公式 SHA-256 検証に対応し、不完全なファイルを取り込みません。
- 専用 Linux 環境の初回起動は現在の WSL 2.7 系に対応し、WSL が入っていなかった Windows 環境からの初回セットアップを実機で確認しています。
...
セットアップ画面で Bluetooth を Windows に戻してから、**アプリ依存関係を削除**を押します。保存されたインストール記録に基づいて Bluetooth の返却と共有解除、専用 Linux 環境の削除、アプリが導入した usbipd の削除を行います。使用前に WSL がなく、他の WSL ディストリビューションも残っていない場合は、SplatoonDeck が追加した WSL ランタイムと Windows 機能も削除します。再起動を求められた場合は、Windows を一度再起動するとクリーンアップが完了します。

## 仕組みと互換性

Windows アプリは画像処理、キーボード・マウス入力、描画進捗を担当します。内蔵のネイティブセットアップ補助プログラムが PowerShell に依存せず、管理者権限と環境管理を処理します。専用の WSL 2 環境は BlueZ と仮想 Pro Controller を実行し、USB/IP、NXBT、Python ブリッジを通じて操作と描画経路を Switch 2 へ送信します。
...
├─ native/     ネイティブ環境管理補助プログラム
├─ scripts/    WSL 内部環境のセットアップ
├─ assets/     ブランドのソースファイル
└─ build/      アプリアイコン
```
